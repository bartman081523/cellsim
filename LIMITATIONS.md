...
## 2. CellsimMixMind-Audit-Status (Stand 2026-09-24, iter-30)
...
| VECTOR_FRESH_SURVIVOR_CENSUS (iter-30) | ✅ abgeschlossen | **B (SURV_ALL\|DEEP3_FRESH_REPLICATED\|EDGE_SIGNALS)** | Abschluss-Zensus der Damköhler-Linie: 100 GPU-Läufe, dritter Seed-Block (310-319, disjunkt zu 200-209 UND 300-309), 4 Anker × frische Kontrollen + 6 Zellen × 10. Gates alle PASS (K1: se_units 4/4 + welch_t D=0.1 k=3 bit-identisch zu iter-28, iter-29-Verankerung per Cross-Session-Determinismus 3/3 — vor dem Lauf mit echten Läufen verifiziert; K2). **SURV_ALL**: alle 4 SE-Überlebenden repliziert, alle positiv (D=0.15 k=5 t=3.17, D=0.3 k=30 t=5.55, D=0.3 k=300 t=4.70, D=0.3 k=1000 t=4.84; LZ +0.035 bis +0.056) — der D=0.30-Hoch-k-Kanal jetzt in VIER Ebenen getragen. **DEEP3_FRESH_REPLICATED**: D=0.1 k=3 auf voll unabhängiger Basis |t|=6.98. **EDGE_SIGNALS (Überraschung)**: die Kante k_edge=10 (D=0.05, an 200-209 unter beiden Regeln NULL) zeigt am dritten Block ein krispes negatives Signal (t=−4.21, Zell-σ 0.0186) — iter-28s Flip-Lesung bleibt als Block-Buchhaltung stehen; die Zelle ist nicht tot, sondern block-fluktuierend. **Neuer Befund**: Effektstärken sind NICHT seed-block-stabil (0.69 SE vs 4.21 SE für dieselbe Zelle) — Signale real, Tiefen Block-Eigenschaften; robuster Kern jetzt 8 Zellen (5 SE + 3 iter-29), alle block-stabil in Richtung. CORREKTUR: Gate-(c)-Redesign vor dem Lauf (iter-29-t_Referenzen nicht per-seed persistierbar → Cross-Session-Determinismus, 3 echte Verifikations-Läufe) — siehe scratch/experiments/iter-30 |
